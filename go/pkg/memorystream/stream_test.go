...
func TestMemoryStream(t *testing.T) {
	...
	_, err := s.Subscribe("domain", streaminterface.MessageHandlerFunc(func(msg streaminterface.Message) error {
		wg.Done()
		return nil
	}))
	if err != nil {
...
func TestMemoryStreamClose(t *testing.T) {
	s := memorystream.New()
	ch := make(chan struct{})
	sub, err := s.Subscribe("domain", streaminterface.MessageHandlerFunc(func(msg streaminterface.Message) error {
		ch <- struct{}{}
		return nil
	}))
	if err != nil {
...
func TestMemoryStreamWithLog(t *testing.T) {
	...
	var wg sync.WaitGroup
	_, _ = s.Subscribe("domain", streaminterface.MessageHandlerFunc(func(msg streaminterface.Message) error {
		wg.Done()
		return nil
	}))
...
func BenchmarkMemoryStream(b *testing.B) {
	...
	var wg sync.WaitGroup
	_, _ = s.Subscribe("domain", streaminterface.MessageHandlerFunc(func(m streaminterface.Message) error {
		msg = m
		wg.Done()
		return nil
	}))
