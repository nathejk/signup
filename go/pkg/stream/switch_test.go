...
func (s *testHandler) HandleMessage(m streaminterface.Message) error {
	return s.handler(m)
}
...
func TestSwitch(t *testing.T) {
	...
	h1 := &testHandler{
		...
		handler: func(m streaminterface.Message) error {
...
func benchmarkSwitchComplexN(b *testing.B, N int) {
	...
	consumer := &testHandler{
		...
		handler:    func(m streaminterface.Message) error { return nil },
	}
	swtch, err := stream.NewSwitch(mux, []streaminterface.Consumer{
		//asset.NewDetailsAggregator(localstream),
		consumer,
